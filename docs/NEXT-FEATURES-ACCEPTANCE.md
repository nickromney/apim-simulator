...
| Deferred items remain deferred: `send-request`, `cache-*`, broader managed identity emulation, broad control-plane emulation, full policy expressions | Deferred capabilities remain unimplemented and are documented as such | `docs/CAPABILITY-MATRIX.md`, `docs/SCOPE.md`, and unsupported fixture `send-request-deferred` reported by `make compat` on `2026-04-02` | `met` |
